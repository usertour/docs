...
![HubSpot asking to approve the permissions Usertour requests](/images/hubspot-11.png)

Installing from the [HubSpot App Marketplace](https://ecosystem.hubspot.com/marketplace/apps)
works the same way from the other end: **Install app** opens Usertour in a
new tab, where you sign in and pick the environment to connect, then
HubSpot asks you to approve the permissions and shows the app as
installed. Either way you end up with one connection per environment.
...
In HubSpot, open the contact's **Activities** tab, open the **Activity**
filter and tick **Usertour** under Integrations — app events are not shown
until their integration is ticked.

![Ticking Usertour under Integrations in a contact's activity filter](/images/hubspot-10.png)

The events then appear on the timeline, each naming the flow, checklist or
launcher it came from.

![Usertour events on a contact's timeline in HubSpot](/images/hubspot-07.png)

The linked company works the same way: tick **Usertour** in its activity
filter, and the same events show up on its timeline.

![Ticking Usertour under Integrations in a company's activity filter](/images/hubspot-09.png)

![Usertour events on a company's timeline in HubSpot](/images/hubspot-08.png)
...
**Disconnect** in the connection card's menu revokes Usertour's access in
HubSpot and stops all syncing — full rounds, incoming changes, write-backs
and timeline events. Nothing is deleted on either side:

- Mappings, links and the message log stay in Usertour, so reconnecting the
  same HubSpot account picks up where you left off.
- Attributes owned by HubSpot keep their values and stay read-only until you
  remove them from a mapping (or remove the mapping), which hands them back
  as ordinary attributes.
- The `Usertour` property group, its values and the timeline events stay in
  HubSpot.

**Uninstalling from HubSpot** (Settings → Integrations → Connected apps →
Usertour → Uninstall) has the same effect: HubSpot stops accepting
Usertour's access, the integration page shows a banner saying so, and
syncing is paused until you click **Reconnect**.

**Removing a mapping** drops its links and hands its HubSpot-owned
attributes back; values are kept on both sides.
