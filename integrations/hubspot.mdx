...
{/* TODO screenshot hubspot-01: Settings → Integrations → HubSpot, the connection card with the "Connect with HubSpot" button, not yet connected. */}
...
{/* TODO screenshot hubspot-02: the same card after connecting, showing the HubSpot account name and id and the Disconnect button. */}
...
{/* TODO screenshot hubspot-03: the Contacts ↔ Users editor dialog with the match row filled in, two or three rows under "Fields to sync from HubSpot" (New badges visible), one row under "Fields to write back to HubSpot", before saving. */}
...
{/* TODO screenshot hubspot-04: a saved mapping card in its read-only state: the match row, the two field sections as pairs, the "Last full sync … · Matched N · Unmatched M" line, and the Run full sync button beside that line. */}
...
Write-backs run through the same pipeline as other integrations, so they
are retried on failure and appear in the **message log** at the bottom of
the page — see [delivery and reliability](/integrations/overview#delivery-and-reliability).
...
{/* TODO screenshot hubspot-05: the attribute picker in a segment or auto-start condition, with a HubSpot-marked attribute (e.g. Lifecycle stage) in the list. */}
...
2. Copy the `integrations/hubspot` directory from the
   ...
   `https://<your-api-host>/integrations/hubspot/oauth/callback`. It must be
   HTTPS and reachable by the browser of the person connecting; HubSpot's
