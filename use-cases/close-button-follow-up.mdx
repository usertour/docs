---
title: Close button follow-up
description: Show a short follow-up tooltip after a user closes or ends an onboarding flow.
icon: "arrows-repeat"
---
...
A good pattern is to let the original flow end normally, then show a short follow-up tooltip that points to the place where onboarding can be reopened.
...
The follow-up flow should not try to continue the onboarding. Its job is smaller: confirm that closing was accepted, and show the user where to return.
...
The follow-up flow uses that recorded event as its start signal. This keeps the behavior tied to the real lifecycle of the original flow, instead of trying to attach a separate action directly to the close button.
