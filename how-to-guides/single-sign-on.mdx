...
**A self-hosted IdP on `localhost` or an internal hostname.** These are supported — Usertour only needs the issuer to be reachable from the server, not publicly resolvable. Use the issuer exactly as it appears in the IdP's `/.well-known/openid-configuration`.
