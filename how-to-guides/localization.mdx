...
<Note>
  Writes replace the stored translation as a whole, last writer wins. If someone has the same language open in the editor while an API write lands, their next autosave overwrites it — coordinate who is translating a language at a time.
</Note>

The languages themselves can be managed the same way — `POST`, `PATCH` and `DELETE` on `/v2/projects/{projectId}/localizations` (MCP: `create_localization`, `update_localization`, `delete_localization`, `restore_localization`), so an agent asked to translate into a language the project doesn't have yet can add it first. These use the `localization:*` scopes, while reading and writing a translation uses the content scopes — see [Scopes](/api-reference-v2/authentication#scopes). Changing the default localization stays in the dashboard.
