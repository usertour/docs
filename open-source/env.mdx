...
| RUN_SEED | No | Whether this instance runs the database seed (idempotent backfills) on startup, default `true`. In multi-instance deployments, set `RUN_SEED=false` on all but one designated instance (or run the seed from a one-off job) so large backfills execute once instead of once per instance. Make sure at least one instance keeps it enabled — new default events and attributes are backfilled by the seed. |
