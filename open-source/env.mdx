...
| API_URL | No | The instance's public base URL (e.g. `https://usertour.example.com`) — the canonical origin used for OAuth/MCP discovery metadata, the SSO callback, and the API/MCP URLs shown in Settings. When unset, these are derived per request from forwarded headers (`Host` / `X-Forwarded-Proto`); set it to pin them when a proxy chain sits in front of the instance. |
...
| TRUST_PROXY | No | Express [`trust proxy`](https://expressjs.com/en/guide/behind-proxies.html) — which upstream proxies to believe when resolving the client IP (rate limiting) and protocol. Default `loopback, linklocal, uniquelocal` trusts the bundled nginx and any in-network proxy, stopping at the first public address. Accepts any value Express supports: a hop count, an address list, or `false` when the Node server is exposed directly. |
