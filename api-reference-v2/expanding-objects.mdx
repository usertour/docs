...
| Content version | `questions`, `steps`, `data`, `localizations` |
...
`localizations` inlines the version's translation status: one entry per
non-default project locale with `enabled`, `missing` and `outdated` counts — the
check to run before publishing a version whose text you edited. See
[Localization](/how-to-guides/localization#translate-via-the-api-or-mcp).
