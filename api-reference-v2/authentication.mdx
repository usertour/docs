...
| `localization:read` | List the project's localizations |
| `localization:create` | Add a localization |
| `localization:update` | Rename / re-code a localization, restore a deleted one |
| `localization:delete` | Delete (soft-delete) a localization |
...
Two endpoints need no scope at all, only a valid token: `GET /v2/me` (what this
token may act on) and `GET /v2/locales` (the locale catalog to file a new
localization under — see [Localization](/how-to-guides/localization#set-up-your-languages)).
