...
<ParamField path="options.token" type="string">
  ...
  environment's signing secret, whose `sub` claim must equal `userId`. Proves
  ...
  enabled for the environment. See [Identity Verification](/developers/identity-verification).
</ParamField>
