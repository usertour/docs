...
- Anonymous users are exempt from [Identity Verification](/developers/identity-verification): this method needs no identity token and keeps working with enforcement on — but anonymous users cannot be associated with a company via `group()`
